...
void Demo::Render() {
	...
	glm::mat4 view = glm::lookAt(glm::vec3(0, 4, 6), glm::vec3(0, 0, 0), glm::vec3(0, 1, 0));
...
void Demo::BuildColoredCube() {
	...
	// format position, tex coords
		...
		-0.5, -0.5, 0.5,   0.0f,  0.0f,  1.0f, // 0
		   1, -0.5, 0.5,   0.0f,  0.0f,  1.0f, // 1
		   1,  2.5, 0.5,   0.0f,  0.0f,  1.0f, // 2
		-0.5,  2.5, 0.5,   0.0f,  0.0f,  1.0f, // 3

		// right
		1,  2.5,  0.5, 1.0f,  0.0f,  0.0f, // 4
		1,  2.5, -0.5, 1.0f,  0.0f,  0.0f, // 5
		1, -0.5, -0.5, 1.0f,  0.0f,  0.0f, // 6
		1, -0.5,  0.5, 1.0f,  0.0f,  0.0f, // 7

		// back
		-0.5, -0.5, -0.5, 0.0f,  0.0f,  -1.0f, // 8 
		 1,   -0.5, -0.5, 0.0f,  0.0f,  -1.0f, // 9
		 1,    2.5, -0.5, 0.0f,  0.0f,  -1.0f, // 10
		-0.5,  2.5, -0.5, 0.0f,  0.0f,  -1.0f, // 11

		// left
		-0.5, -0.5, -0.5, -1.0f,  0.0f,  0.0f, // 12
		-0.5, -0.5,  0.5, -1.0f,  0.0f,  0.0f, // 13
		-0.5,  2.5,  0.5, -1.0f,  0.0f,  0.0f, // 14
		-0.5,  2.5, -0.5, -1.0f,  0.0f,  0.0f, // 15

		// upper
		   1, 2.5,  0.5, 0.0f,  1.0f,  0.0f, // 16
		-0.5, 2.5,  0.5, 0.0f,  1.0f,  0.0f, // 17
		-0.5, 2.5, -0.5, 0.0f,  1.0f,  0.0f, // 18
		   1, 2.5, -0.5, 0.0f,  1.0f,  0.0f, // 19

		// bottom
		-0.5, -0.5, -0.5,  0.0f,  -1.0f,  0.0f, // 20
		   1, -0.5, -0.5,  0.0f,  -1.0f,  0.0f, // 21
		   1, -0.5,  0.5,  0.0f,  -1.0f,  0.0f, // 22
...
void Demo::DrawColoredPlane()
{
	...
	// set lighting materials attributes (pearl)
	GLint ambientMatLoc = glGetUniformLocation(this->activeProgram, "material.ambient");
	glUniform3f(ambientMatLoc, 0.25f, 0.20725f, 0.20725f);
	GLint diffuseMatLoc = glGetUniformLocation(this->activeProgram, "material.diffuse");
	glUniform3f(diffuseMatLoc, 1.0f, 0.829f, 0.829f);
	GLint specularMatLoc = glGetUniformLocation(this->activeProgram, "material.specular");
	glUniform3f(specularMatLoc, 0.296648f, 0.296648f, 0.296648f);
	GLint shininessMatLoc = glGetUniformLocation(this->activeProgram, "material.shininess");
	glUniform1f(shininessMatLoc, 0.088f);
...
void Demo::BuildTexturedCube()
{
	...
	int width, height;
	unsigned char* image = SOIL_load_image("Refrigerator_diffusemap.png", &width, &height, 0, SOIL_LOAD_RGBA);
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	image = SOIL_load_image("Refrigerator_specularmap.png", &width, &height, 0, SOIL_LOAD_RGBA);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, image);
	SOIL_free_image_data(image);
	glBindTexture(GL_TEXTURE_2D, 0);
	...
	// ------------------------------------------------------------------
	float vertices[] = {
		// format position, tex coords, normals
		// front
		-0.5, -0.5, 0.5, 0,0,   0.0f,  0.0f,  1.0f, // 0
		   1, -0.5, 0.5, 1,0,   0.0f,  0.0f,  1.0f, // 1
		   1,  2.5, 0.5, 1,1,   0.0f,  0.0f,  1.0f, // 2
		-0.5,  2.5, 0.5, 0,1,   0.0f,  0.0f,  1.0f, // 3

		// right
		1,  2.5,  0.5, 0,0, 1.0f,  0.0f,  0.0f, // 4
		1,  2.5, -0.5, 1,0, 1.0f,  0.0f,  0.0f, // 5
		1, -0.5, -0.5, 1,1, 1.0f,  0.0f,  0.0f, // 6
		1, -0.5,  0.5, 0,1, 1.0f,  0.0f,  0.0f, // 7

		// back
		-0.5, -0.5, -0.5, 0,0, 0.0f,  0.0f,  -1.0f, // 8 
		 1,   -0.5, -0.5, 1,0, 0.0f,  0.0f,  -1.0f, // 9
		 1,    2.5, -0.5, 1,1, 0.0f,  0.0f,  -1.0f, // 10
		-0.5,  2.5, -0.5, 0,1, 0.0f,  0.0f,  -1.0f, // 11

		// left
		-0.5, -0.5, -0.5, 0,0, -1.0f,  0.0f,  0.0f, // 12
		-0.5, -0.5,  0.5, 1,0, -1.0f,  0.0f,  0.0f, // 13
		-0.5,  2.5,  0.5, 1,1, -1.0f,  0.0f,  0.0f, // 14
		-0.5,  2.5, -0.5, 0,1, -1.0f,  0.0f,  0.0f, // 15

		// upper
		   1, 2.5,  0.5, 0,0, 0.0f,  1.0f,  0.0f, // 16
		-0.5, 2.5,  0.5, 1,0, 0.0f,  1.0f,  0.0f, // 17
		-0.5, 2.5, -0.5, 1,1, 0.0f,  1.0f,  0.0f, // 18
		   1, 2.5, -0.5, 0,1, 0.0f,  1.0f,  0.0f, // 19
...
void Demo::BuildTexturedPlane()
{
	...
	int width, height;
	unsigned char* image = SOIL_load_image("Plane_diffusemap.png", &width, &height, 0, SOIL_LOAD_RGBA);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, image);
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	image = SOIL_load_image("Plane_specularmap.png", &width, &height, 0, SOIL_LOAD_RGBA);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, image);
	SOIL_free_image_data(image);
	glBindTexture(GL_TEXTURE_2D, 0);
...
void Demo::DrawTexturedCube()
{
	...
	GLint shininessMatLoc = glGetUniformLocation(this->activeProgram, "material.shininess");
	glUniform1f(shininessMatLoc, 15.0f);
